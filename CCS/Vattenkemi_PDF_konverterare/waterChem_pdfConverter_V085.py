...
#myPath="C:/Calluna/Data/SandBox/Convert_PDF_Table/Analyssvar/2022-04-12_13"
myPath = r"C:/Calluna/Data/SandBox/Convert_PDF_Table/Analyssvar/Fria vattenmassan 2022/Juni/Kontrollområde CeKo/Kontrollerade prover/CeKo_H allt ok"
os.chdir(myPath) 
...
#aPdf="C:/Calluna/Data/SandBox/Convert_PDF_Table/Analyssvar/2022-04-12_13/A botten_177-2022-04141303_01.pdf"
...
print(f"Exporting Vattenkemi_FranPdf_{myPath}.csv \n\nto directory: {myPath}")
...
aPdf=r"C:\Calluna\Data\SandBox\Convert_PDF_Table\Analyssvar\Test\CePå_Y botten (177-2022-06110165)_177-2022-06301357_01.pdf"
dfs = ti.read_pdf(aPdf, pages = "all")
df = dfs[aTbl]
df = tableAdjustment(df)
...
dateDf=dfSt.loc[dateMask.apply(lambda x:any(x), axis = 1),dateMask.apply(lambda x:any(x))] #Use two apply functions to identify the true position in row and column index
if(sum(dateDf.shape) == 0):
   print("Looking for alternative date") 
   dateMaskAlt=dfSt.apply(lambda x: x.str.contains("(?i)(?<=prov).+?\d{4}-\d{2}-\d{2}$", regex = True), axis = 1)
   dateDfAlt=dfSt.loc[dateMaskAlt.apply(lambda x:any(x), axis = 1),dateMaskAlt.apply(lambda x:any(x), axis = 0)]
   dateVal=re.search('\d{4}-\d{2}-\d{2}', str(dateDfAlt.iloc[0,0])).group(0)
else:
...
mtchPrv=re.search("(?<=Provnummer: ).*",df.columns.to_list()[0])
dfCh.rename(columns={dfCh.columns[0]: "Analys"}, inplace=True)
if(sum(dfCh.columns == "Ankomsttemp °C Kem") == 0):
    digPatn = '^((<|>)\d+\.\d+$|^\d+\.\d$|^\d+$|^\d+\,\d$)'    
    untPatn = '\w+\/l'
    for colPos, colVal in zip(range(0,len(dfCh.columns)),dfCh.columns): 
        try:
...
#r'(?i)Prov\w+\s+(\d{4}-\d{2}-\d{2})'
#(?i)(Prov\w+\s+\w+:)\s+\d{4}-\d{2}-\d{2}

# =============================================================================
# testPrd=frontTest(dfs[0])
# 
#  = dfs[0]
# 
# df = tableAdjustment(dfs[0])
# 
# colID=df.filter(regex=("Provnummer.*")).columns.to_list()
# startRow=df.loc[(df[colID[0]] == 'Analys')|(df[colID[0]] == 'Analys ')].index.to_numpy()
# dfCh=df.iloc[startRow[0]+1:df.shape[0],:]
...
#    dateMaskAlt=dfSt.apply(lambda x: x.str.contains("(?i)(?<=prov).+?\d{4}-\d{2}-\d{2}$", regex = True), axis = 1)
#    dateDfAlt=dfSt.loc[dateMaskAlt.apply(lambda x:any(x), axis = 1),dateMaskAlt.apply(lambda x:any(x), axis = 0)]
#    dateDfAlt=re.search('\d{4}-\d{2}-\d{2}', str(dateDfAlt.iloc[0,0])).group(0)
#    
# type(dateDfAlt)
# =============================================================================
...
#         #print("First!")
#         dfs[aTbl] = tableAdjustment(dfs[aTbl])
#         dfs[aTbl].to_csv("Working_Table.csv", index = 0)
#         dfs[aTbl] = frontPage(dfs[aTbl])
#         logicGate = False
#         
# =============================================================================